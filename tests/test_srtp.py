...
class TestSRTPPacketEncoding:
    def test_data_packet_encode_decode(self):
        """data_packet encodes and decodes correctly"""
    ...
    def test_ack_packet_encode_decode(self):
        """ ack packet encodes and decodes correctly (and is recognised as ack)"""
    ...
    def test_end_packet_detection(self):
        """ end packet is correctly recognised"""
...
class TestSRTPCRCErrorDetection:
    def test_corrupted_packet(self):
        p = SRTPPacket(1, 5, 8, 42, 12345678, b'Hi SRTP!')
...
def start_server(port, root=None):
...
def start_link_sim(*args):
    return subprocess.Popen(
        [LINK_SIM] + list(args),
        stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL,
    )

def run_client(url, save_path, timeout=30):
...
class TestSRTPFileTransfer:
    def test_basic_file_transfer(self, tmp_path):
        content = b"Hello SRTP! basic_file_transfer\n" * 100
        src = tmp_path / "test_input.txt"
        dst = tmp_path / "test_output.txt"
        src.write_bytes(content)
        server = start_server(8080, root=tmp_path)
        time.sleep(1)

        try:
            result = run_client(f"http://[::1]:8080/{src.name}", dst)
            assert result.returncode == 0, result.stderr.decode()
            assert dst.exists(), "missing output"
    ...
    def test_server_root_option(self, tmp_path):
        content = b"Hello SRTP! server_root_option\n"
        (tmp_path / "custom.txt").write_bytes(content)
        dst = tmp_path / "custom_output.txt"
        server = start_server(8080, root=tmp_path)
        time.sleep(1)

        try:
            result = run_client("http://[::1]:8080/custom.txt", dst, timeout=10)
    ...
    def test_missing_file_returns_error(self, tmp_path):
        ...
        try:
            result = run_client("http://[::1]:8080/does_not_exist.txt", dst, timeout=10)
    ...
    def test_save_option(self, tmp_path):
        content = b"Hello SRTP! test_save_option\n"
        ...
        try:
            result = run_client(f"http://[::1]:8080/{src.name}", dst, timeout=10)
...
class TestSRTPLinkSim:
    ...
    def _run_link_test(self, tmp_path, filename, link_args, port, timeout=180):
        ...
        server = start_server(port, root=tmp_path)
        time.sleep(1)
        link = start_link_sim(*link_args)
        time.sleep(1)

        try:
            result = run_client(f"http://[::1]:{port}/{src.name}", dst, timeout=timeout)
    ...
    def test_seqnum_wraparound(self, tmp_path):
        ...
        server = start_server(12351, root=tmp_path)
        time.sleep(1)

        try:
            result = run_client(f"http://[::1]:12351/{src.name}", dst, timeout=300)
